...
}).superRefine((val: CreateApiKeyFormProps, ctx: RefinementCtx) => {
  if (!val.isGlobalAdmin && (val.entityScopes?.length ?? 0) === 0) {
    ctx.addIssue({
      code: 'custom',
      message: 'At least one entity scope is required for non-global admin API keys',
      path: ['entityScopes'],
    })
  }
})
            ...
            <>
              ...
              {!formState.isGlobalAdmin && (
                <EditUserScopes
                  ...
                  emptyStateMessage={
                    'At least one scope is required for non-global admin API keys.'
                  }
                  onChange={(entityScopes) => {
                    if (!identity.isGlobalAdmin) {
                      ...
                      const selectedOrganizationIds =
                        entityScopes?.map((scope) => scope.entityId) ?? []

                      if (selectedOrganizationIds.length > 0) {
                        const hasPermissionForAnySelected = selectedOrganizationIds.some(
                          (id) => allowedOrganizationIds.includes(id)
                        )
